...
public class CrawlingMarkupHandler extends AbstractSimpleMarkupHandler {
    ...
    */
    public void handleDocumentStart(long startTimeNanos, int line, int col) {
        // TODO: Implement this.
        wordLocation = 0;
        wordLocationList = new ArrayList<>();
    ...
    */
    public void handleOpenElement(String elementName, Map<String, String> attributes, int line, int col) {
        if(elementName.equals("script") || elementName.equals("style")) scriptStyleTags++;
        if(scriptStyleTags == 0 && elementName.equals("a") && attributes.containsKey("href")) {
            try {
                URL url = new URL(currentPage.getURL(), attributes.get("href"));
                if (url.toString().endsWith(".html") || url.toString().endsWith(".htm")) {
                    /*
                    if(attributes.get("href").startsWith(".")) {
                        String updatedURL = attributes.get("href").replaceAll("\\.\\./","");
                        System.out.println(updatedURL);
                    }
                     */
                    if(!URLs.contains(url)){
                        newURLs.add(url);
                        URLs.add(url); //just to make sure no cycles of URLs
                    }
                }
    ...
    */
    public void handleText(char[] ch, int start, int length, int line, int col) {
        ...
        if(scriptStyleTags == 0) {
            ...
            String[] text = allText.toString().split("\\s+|\\W|_");

            for (String word : text) {
                if (!word.isBlank()) { //even though \\s+ should split on all whitespace, for some reason some is leftover so
                    webIndex.addWord(word.toLowerCase(), currentPage, wordLocation);
                    webIndex.addPage(currentPage, word.toLowerCase());
                    wordLocation++;

                    //System.out.println("word: " + word);
                    //System.out.println("current page: " + currentPage.getURL());
                    //System.out.println("word location: " + wordLocation);
                }
            }
        }
    }
